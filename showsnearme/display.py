...
def print_shows(
    shows, chunk_days=False, n_shows_daily=None, show_url=True, show_eta=False, **kwargs
):
    if not chunk_days:
        for show in shows:
        ...
        future_strings = {0: "Today", 1: "Tomorrow", 2: "Day after tomorrow"}
        first_iter = True
        for day, day_shows in shows_groups:
            ...
            for i, show in enumerate(day_shows):
                if n_shows_daily and i >= n_shows_daily:
                    ...
                    break
                display = format_show(show, show_url=show_url, show_eta=show_eta)
                print(f"\t{display}")
            first_iter = False
