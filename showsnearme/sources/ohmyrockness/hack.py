...
FIND_AUTHORIZATION = re.compile(b"""['"]([0-9a-f]{32})['"].*?Authorization""")


def get_authorization_token():
    """ I sure am sorry about this. """
    main_page = requests.get(BASE_URL).content
    application_path = FIND_APPLICATION.search(main_page).groups()[0].decode("utf8")
    full_application_path = urljoin(BASE_URL, html.unescape((application_path)))
